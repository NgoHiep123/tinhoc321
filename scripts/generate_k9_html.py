...
def read_csv_file(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            # Lấy mã bài từ q_id (vd: K9B1_01 -> B1)
            q_id = row.get('q_id', '')
            if q_id:
                # Xử lý: K9B1_01 -> B1
...
def main():
    """Hàm chính"""
    print("=" * 60)
    print("TẠO HTML ĐẦY ĐỦ CHO KHỐI 9")
    print("=" * 60)
    
    output_dir = '.'
    csv_dir = 'Bai_tap_Tin_9'
    
    # Tìm tất cả file CSV trong thư mục
    csv_files = sorted([f for f in os.listdir(csv_dir) if f.endswith('.csv') and 'K9_question_Bai' in f])
    ...
    for csv_file in csv_files:
        ...
        if not grouped:
            ...
            if lesson_code and questions:
                grouped[lesson_code] = questions
                print(f"  - Sử dụng mã bài từ tên file: {lesson_code}")
